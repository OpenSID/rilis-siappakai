<div class="col-md-9">
    <div class="row">
        <div class="col-12">
            <div class="card">
                            ...
                            <tbody>
                                @foreach ($pelanggans as $index => $item)
                                    <tr>
                                        ...
                                        <td><a href="{{ substr($item->domain_opensid, 0, 8) == 'https://' ? $item->domain_opensid : 'https://' . $item->domain_opensid }}"
                                                target="_blank">
                                                {{ substr($item->domain_opensid, 0, 8) == 'https://' ? $item->domain_opensid : 'https://' . $item->domain_opensid }}</a>
                                        </td>
                                        <td><a href="{{ substr($item->domain_pbb, 0, 8) == 'https://' ? $item->domain_pbb : 'https://' . $item->domain_pbb }}"
                                                target="_blank">
                                                <i class="fa fa-external-link" aria-hidden="true"></i>
                                            </a>
                                        </td>
                                        <td><a href="{{ substr($item->domain_api, 0, 8) == 'https://' ? $item->domain_api : 'https://' . $item->domain_api }}"
                                                target="_blank">
                                                <i class="fa fa-external-link" aria-hidden="true"></i>
                                            </a>
                                        </td>
                ...
                <div class="card-footer clearfix">
                    <a href="{{ route('pelanggan.index') }}" class="btn btn-sm btn-secondary float-right">Lihat
                        Selengkapnya</a>
                    <a href="{{ route('pelanggan.create') }}"
                        class="btn btn-sm btn-secondary float-right me-2 {{ env('OPENKAB') == 'true' ? '' : 'd-none' }}">Tambah
                        {{ $sebutandesa }}</a>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-transparent">
                    <h3 class="card-title">Jumlah Desa Per Kabupaten </h3>
                    <div class="card-tools">
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                        </button>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="datatable" class="table table-striped table-bordered datatable">
                            ...
                            <tbody>
                            @foreach ($jumlahperkab as $key => $item )
                                <tr>
                                <td class="text-center">{{ $key + 1 }}</td>
                                <td class="text-center">{{ $item->kode_kabupaten }}</td>
                                <td class="text-left">{{ $item->nama_kabupaten }}</td>
