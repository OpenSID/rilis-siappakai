<x-app-layout title="Ubah {{ ucwords(str_replace('-', ' ', $table )) }}">
    ...
    <x-breadcrumbs navigations="Pengaturan {{ ucwords(str_replace('-', ' ', $table )) }}"
        active="Ubah {{ ucwords(str_replace('-', ' ', $table )) }}" link="{{ route($table.'.index') }}"></x-breadcrumbs>
    @endsection
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route($table.'.update', encrypt($tema->id)) }}" method="post"
                        ...
                        <span style="float: right; margin-top: -40px">
                            ...
                            <button type="button" class="btn btn-danger btn-circle" data-toggle="modal"
                                data-target="#{{ $table }}-{{ $tema->id }}">
                                <i class="fa fa-trash"></i> Hapus
                            </button>

                            <!-- Modal Hapus Data -->
                            @include('layouts.modals.delete', ['table' => $table , 'data' => $tema])
                        </span>
                    </div>
                </div>
            </div>
