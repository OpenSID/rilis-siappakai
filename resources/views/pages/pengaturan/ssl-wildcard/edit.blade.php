            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route($table.'.update', encrypt($certificate->id)) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                            ...
                            <span style="float: right; margin-top: -40px">
                                <!-- Tombol Hapus Data -->
                                <button type="button" class="btn btn-danger btn-circle" data-toggle="modal" data-target="#{{ $table }}-{{ $certificate->id }}">
                                    <i class="fa fa-trash"></i> Hapus
                                </button>

                                <!-- Modal Hapus Data -->
                                @include('layouts.modals.delete', ['table' => $table , 'data' => $certificate])
                            </span>
                        </div>
                    </div>
                </div>
